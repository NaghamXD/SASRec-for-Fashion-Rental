...
def build_index(dataset_name):
    # FIX: Handle absolute paths passed from train_lg.py
    if os.path.isabs(dataset_name):
        path = dataset_name + '.txt'
    else:
        path = 'data/%s.txt' % dataset_name

    ui_mat = np.loadtxt(path, dtype=np.int32)
